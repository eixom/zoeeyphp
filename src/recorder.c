...
static zval * to_indexed(zval * sql, zval ** keys, char * query, int len TSRMLS_DC) {
    char   ch            = 0;
    int    i             = 0;
...
PHP_METHOD(ze_recorder, __construct) {
    ...
    if (conn && Z_TYPE_P(conn) == IS_OBJECT) {
        zend_update_property(ze_recorder_ce, self, ZEND_STRL(ZE_CONN) , conn TSRMLS_CC);
    } else {
        /* PDO is not connected */
        ze_error(E_ERROR TSRMLS_CC, "recorder.construct.conn.not_object");
    }
...
PHP_METHOD(ze_recorder, query) {
   ...
   if (zend_parse_parameters(ZEND_NUM_ARGS() TSRMLS_CC, "s"
    ...
    conn = zend_read_property(ze_recorder_ce, self, ZEND_STRL(ZE_CONN), 0 TSRMLS_CC);
    ...
    do {
        ...
        to_indexed(sql, &keys, query, query_len TSRMLS_CC);
...
PHP_MINIT_FUNCTION(ze_recorder) {
    ...
    zend_declare_property_null(ze_recorder_ce, ZEND_STRL(ZE_CONN) , ZEND_ACC_PRIVATE TSRMLS_CC);

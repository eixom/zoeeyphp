...
/*
        spprintf(&sep, 1, "%s", pattern + len - 2);
*/
        ...
        if (len > 3 && left == '[' && right == ']') {
/*
            spprintf(&pattern, len - 3, "%s", pattern);
*/
            offset = NULL;
        ...
        for (zend_hash_internal_pointer_reset_ex(rules_table, &rules_pointer);
            ...
            switch (type) {
                ...
                case ZE_TYPE_COUNT:
                    ...
                    count_query = estrdup(Z_STRVAL(query_z));
                    counter = 0;
                    count_offset = NULL;
            ...
            if (is_parse_sep) {
                ...
                while (pattern_part != NULL) {
                    if (!query_offset) {
                        value = php_strtok_r(Z_STRVAL(query_z), Z_STRVAL_PP(sep_z), &query_offset);
                    } else {
                    ...
                    parse_pattern(values, Z_STRVAL(name_z), Z_STRLEN(name_z), value TSRMLS_CC);
                    ZE_DTOR(name_z);
